...
impl CommitCheckerFacade {
    ...
    pub fn analyze(&mut self, file_name: &str, file_content: &str){
        ...
        let file_context = FileContext::new(file_name.to_string(), file_content, &allocator, &mut self.container.message_handler);
        ...
        println!("{:?}", self.container.message_handler.get_all_messages());
    }
}